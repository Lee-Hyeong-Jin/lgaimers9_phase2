...
def run(FF, fl, name):
    ...
    rr = y[va][isR].mean()
    print(f'[fold {vs}] {name:36s} all: raw={bss_score(y[va], ps_):.1f} oracle={bss_score(y[va], oracle_shift(ps_, r)[0]):.1f} | R-only oracle={bss_score(y[va][isR], oracle_shift(ps_[isR], rr)[0]):.1f}', flush=True)
    return ps_
p_ref = run(F, feats, 'reference (absolute view)')
p_dev = run(Fd, feats, 'deviation view')
p_devb = run(Fd, feats + ['bayes_dev', 'bayes_pv', 'bayes_m0', 'bayes_v0'], 'deviation view + bayes')
p_absb = run(deviation_view(F, seas, means.iloc[0:0].reindex(means.index) * 0 + 0, bay) if False else F.assign(**bay), feats + ['bayes_dev', 'bayes_pv', 'bayes_m0', 'bayes_v0'], 'absolute view + bayes')
os.makedirs(os.path.join(EXP, 'oof'), exist_ok=True)
np.save(os.path.join(EXP, 'oof', f'mcs_dev_{vs}.npy'), p_dev); np.save(os.path.join(EXP, 'oof', f'mcs_devb_{vs}.npy'), p_devb); np.save(os.path.join(EXP, 'oof', f'mcs_absref_{vs}.npy'), p_ref)
